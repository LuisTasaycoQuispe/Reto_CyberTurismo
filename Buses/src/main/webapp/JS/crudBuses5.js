...
function fnBtnInactivos() {
	let ctrlPlaca = document.getElementById("ctrlPlaca").value;
	let ctrlCodigo = document.getElementById("ctrlCodigo").value;

	let url = "BusesListarInactivo?placa=" + ctrlPlaca;
	url += "&codigo=" + ctrlCodigo;
	let xhttp = new XMLHttpRequest();
	xhttp.open("GET", url, true);
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
